...
pub async fn upload_file_with_retry(
    ...
    Retry::spawn(retry_strategy, || async {
        upload_file(file_properties, sub_folder, b2, client).await
    })
    .await
}
...
async fn upload_file(
    ...
    let upload_url_result = get_upload_url_with_retry(b2, client).await;
...
pub async fn get_upload_url_with_retry(
    ...
    Retry::spawn(retry_strategy, || async {
        get_upload_url(b2, client).await
    })
    .await
}
...
async fn check_token(b2: &Arc<RwLock<B2>>, client: &reqwest::Client) {
    ...
    if _b2.token_time.elapsed().as_secs() > 43200 {
        ...
        match b2.check_token(client).await {
            Ok(_) => {}
